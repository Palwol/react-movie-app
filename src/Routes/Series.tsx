...
import { getSeries, ISeriesResult } from "./api";
...
function Series() {
  ...
  const { data: popularData, isLoading } = useQuery<ISeriesResult>(
    ["tv", "popular"],
    () => getSeries("popular")
  );
  const { data: topData } = useQuery<ISeriesResult>(["tv", "top"], () =>
    getSeries("top_rated")
  );
        ...
        <>
          ...
          >
            ...
            <BannerBtns>
              ...
              <BannerDetails
                onClick={() => onBoxClicked(popularData?.results[0].id)}
              >
                <DetailSvg />
                <span>Details</span>
              </BannerDetails>
